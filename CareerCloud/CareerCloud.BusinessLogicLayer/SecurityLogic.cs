...
{
    public class SecurityLogic : BaseLogicRepository<SecurityLoginPoco>
    {
        private const int saltLengthLimit = 10;

        public SecurityLogic(IGenericRepository<SecurityLoginPoco> repository) : base(repository)
        {
        }

        public bool Authenticate(string userName, string password)
        {
            SecurityLoginPoco poco = base.GetAll().Where(s => s.LoginName == userName).FirstOrDefault();
            if (null == poco)
            {
                return false;
            }
        ...
        public override void Add(SecurityLoginPoco poco)
        {
             Verify(poco);
        ...
        protected override void Verify(SecurityLoginPoco poco)
        {
